...
The raw signal measured in the acceptor's channel under donor excitation ($I_{DA}$) needs correction to accurately estimate $F_{c}$. This correction is essential for subtracting unwanted contributions of the signals from direct acceptor excitation crosstalk and donor signal bleedthrough into the acceptor channel. These necessary corrections utilize the signals from the acceptor ($I_{AA}$) and donor ($I_{DD}$) spectral channels, along with their corresponding coefficients, $a$ and $d$ (eq. \autoref{eq:f_c}).
...
- __FRET efficiency__ ($E_{D}$) estimation: This is achieved using the E-FRET approach, which normalises the FRET signal to the total donor intensity ($I_{D}^{tot}$) (eq. \autoref{eq:e_d}).

$$E_{D} = \frac{F_{c}}{I_{D}^{tot}} = \frac{F_c / G}{F_c / G + I_{DD}} \label{eq:e_d}$$

- __FRET ratio__ ($E_{A}$) estimation: This is achieved using the 3^3^-FRET approach, which is based on the total acceptor intensity ($I_{A}^{tot}$) (eq. \autoref{eq:e_a}).
